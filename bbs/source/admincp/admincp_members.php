 ...
 *
 *      $Id: admincp_members.php 19347 2010-12-28 08:07:12Z zhengqingpeng $
 */
	...
	if(!submitcheck('submit', 1)) {
		...
		if(!$_G['gp_confirmed']) {
			...
			if(empty($_G['gp_includepost'])) {
				...
				$nextlink = "action=members&operation=clean&confirmed=yes&submit=yes&includepost=yes".(!empty($_G['gp_includeuc']) ? '&includeuc=yes' : '')."&current=$next&pertask=$pertask&lastprocess=$processed".$urladd;
				if($threads) {
